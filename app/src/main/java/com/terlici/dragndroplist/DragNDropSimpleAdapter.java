...
public class DragNDropSimpleAdapter extends SimpleAdapter implements DragNDropAdapter {
	...
	public void onItemDrop(DragNDropListView parent, View view, int startPosition, int endPosition, long id) {
		...
		if (startPosition < endPosition) { for (int i = startPosition; i < endPosition; ++i) { mPosition[i] = mPosition[i + 1]; } }
		else if (endPosition < startPosition) { for (int i = startPosition; i > endPosition; --i) { mPosition[i] = mPosition[i - 1]; } }
